//! The uniform daemon shell: argv, binding, the two listener tiers, the
//! connection spine, and the exit report.
//!
//! This was emitted by `schema-rust`'s daemon emitter while that emitter
//! existed. `schema-rust` has since deleted it, so the shell is introspect's
//! own source: nothing here is component-specific, and everything
//! component-specific reaches it through `ComponentDaemon`.
//!
//! The component declares its `Configuration` / `Engine` / `Error` types and
//! fills the hooks; the shell owns argv parsing, async task-backed
//! multi-listener binding, request gating, peer credentials, lifecycle, and
//! the `ExitReport` entry.
...
/// The component hook surface for the daemon shell — the only daemon
...
/// wraps it in the shell's actor connection runtime, and returns the
/// async task-backed listener shell the `DaemonCommand` drives. The component
/// never writes this by hand — it is a default method on
/// `ComponentDaemon`.
pub trait DaemonBinder: ComponentDaemon {
    fn bind(
        configuration: Self::Configuration,
    ) -> Result<AsyncMultiListenerDaemon<DaemonConnectionRuntime<Self>>, DaemonError<Self>> {
        let engine = Self::build_runtime(&configuration).map_err(DaemonError::Component)?;
        let runtime = DaemonConnectionRuntime::<Self>::new(engine);
        Ok({
...
pub struct DaemonConnectionRuntime<Daemon: ComponentDaemon> {
    engine: std::sync::Arc<Daemon::Engine>,
}
impl<Daemon: ComponentDaemon> DaemonConnectionRuntime<Daemon> {
...
impl<Daemon: ComponentDaemon> Clone for DaemonConnectionRuntime<Daemon> {
    fn clone(&self) -> Self {
        Self {
...
impl<Daemon: ComponentDaemon> AsyncMultiConnectionRuntime for DaemonConnectionRuntime<Daemon> {
...
/// The daemon shell error: argv, configuration, listener, and the
